		...
		actors: [
			{
				...
				birthday: '19-07-1980',
				country: 'US',
				directors: [{ name: 'Robert Uppney Sr', birthday: '19-07-1920', country: 'US' }],
			},
		],
	},
];
...
const resolvers = {
	Query: {
		async movies(_, args, { user }) {
			...
			const moviesWithRating = movies.map(
				m => (m.scoutbase_rating = String((Math.floor(Math.random() * 39) + 50) / 10)) && m
			);
